...
public class DeployControllerJunit  implements Filter{
	private static Logger logger=LoggerFactory.getLogger(DeployControllerJunit.class);
	...
	public void process(FilterChain chain) throws Exception {
		logger.debug("begin---开始部署controller测试类");
		...
		controllerPath=StringUtil.testPackageToPath(controllerPackage);
		File viewFile=null;
		if(path!=null){
		...
		for(File f:all){
			if(f.getAbsolutePath().endsWith(suffix)){
				...
				Global.FU.process("ControllerJunit", chain.getRoot(), controllerPath+controllerName+"ControllerTest.java");
				logger.debug("成功部署{}ControllerTest.java",controllerName);
			}
		}
		logger.debug("end---成功部署所有controller测试类");
